**TITLE**
Analysis, Code, and Data for 9-Week Moving Averages of Stream Chemistry Following Hurricane Hugo

**REPO'S PURPOSE**
This repository contains the raw data and R code used to reproduce Figure 3 from Schaefer et al. (2000). Figure 3 shows 9-week moving averages of stream-water chemistry before and after Hurricane Hugo across forested watersheds in the Luquillo Experimental Forest in Puerto Rico.

This repository will be updated to reproduce the 9-week moving averages for the stream chemistry variables included in Figure 3:
- Potassium 
- Nitrate-N
- Magnesium
- Calcium
- Ammonium-N

The analysis focuses on the period from 1988 through 1994, allowing us to examine changes in stream chemistry before and after Hurricane Hugo.

**REPOSITORY CONTENTS**
The repository currently includes the raw stream chemistry data and the R code used for the analysis.

The four watershed datasets are represented in the analysis as:
- BQ1 – Quebrada Cuenca 1-Bisley
- BQ2 – Quebrada Cuenca 2-Bisley
- BQ3 – Quebrada Cuenca 3-Bisley
- PRM – RíoMameyesPuente Roto

The analysis calculates 9-week moving averages of stream chemistry measurements and plots these averages over the period from 1988 – 1994.

At this time, the repository includes an analysis and figure showing the 9-week moving average of potassium (K, mg/L)over time (1990 - ).

The repository will be updated with analyses of nitrate-N, magnesium, calcium, and ammonium-N as the course progresses.

**DETAILS REGARDING DATA ACCESS**
Data was downloaded from the Environmental Data Initiative (EDI) Data Portal. 

McDowell, W. and International Institute of Tropical Forestry(IITF), USDA Forest Service.. 2024. Chemistry of stream water from the Luquillo Mountains ver 4923064. Environmental Data Initiative. https://doi.org/10.6073/pasta/f31349bebdc304f758718f4798d25458 (Accessed 2026-08-25).

**A LIST OF AUTHORS**
Farah Stack

**REFERENCES**
Schaefer, Douglas. A., William H. McDowell, Fredrick N. Scatena, and Clyde E. Asbury. 2000. “Effects of Hurricane Disturbance on Stream Water Concentrations and Fluxes in Eight Tropical Forest Watersheds of the Luquillo Experimental Forest, Puerto Rico.” Journal of Tropical Ecology 16 (2): 189–207. https://doi.org/10.1017/s0266467400001358.